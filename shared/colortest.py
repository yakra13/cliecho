...
if __name__ == "__main__":
    # COLOR_SUPPORT = ColorSupport.ANSI_16
    # original_start = Color.White
    # original_end = Color.Red
    start_color = Color(120, 0, 0)
    end_color = Color.Red
    sparkle_start = Color.LightGray
    sparkle_end = Color.White
    ...
    while True:
        ...
        for y in range(height):
            for x in range(width):
                c = logo[y][x]
                # if c == '*':
                #     output.append(style_text(c, lerp_color(sparkle_start, sparkle_end, (elapsed_time % 30) / 30), styles=[TextStyle.BOLD]))
                if x > 29:
                    output.append(style_text(c, lerp_color(Color.Black, Color.DarkRed, y / (height - 1)), back_color=Color.DarkRed, styles=[TextStyle.BOLD]))
                else:
                    new_color = lerp_color(start_color, end_color, current_t)
                    fade_color = lerp_color(new_color, Color(30, 0, 0), y / (height - 1))
                    output.append(style_text(c, fade_color, styles=[TextStyle.BOLD]))

            output.append("\n")

        
        output = "".join(output)
        sys.stdout.write('\033[8A\033[K' + output)
        sys.stdout.flush()
        time.sleep(0.05)

        current_t += step * direction
        if current_t >= 1.0 or current_t < 0:
            direction *= -1
